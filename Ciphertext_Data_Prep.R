...
ggplot(data = as.data.frame(head(table(dtm_sort$Frequency),25)), aes(x=Var1, y=Freq))+
  ...
  labs(title = "Term Frequency", x= "Term Repitition", y= "Term Frequency")+
  theme_minimal()
...
levels(train_chr$Var1)[match(" ", levels(train_chr$Var1))]<-"space"
...
levels(test_chr$Char)[match(" ", levels(test_chr$Char))]<-"space"
...
test_plot_top <- ggplot(head(comp_test[order(-comp_test$Prop),],30),
  ...
  labs(title = "Most Frequent Characters in Test-Sets 1 & 2", 
       x ="Character",
       y= "Percentage")+
  scale_y_continuous(labels = scales::percent_format(accuracy = 1))+
  theme_minimal()
...
#Plotting the Padded Character Counts
chr_freq_adj<- rbind(ChrFreqTbl(train), ChrFreqTbl(test1))

ggplot(chr_freq_adj[chr_freq_adj$chr_cnt>100,], aes(chr_cnt, counts, fill = source))+
  geom_bar(stat = "identity")+
  labs(title = "Character Counts (w/Padding) Over 100: \nTrain and Test1 Sets", y= "Character Counts Per Line", x= "Frequency")+
  theme_minimal()
